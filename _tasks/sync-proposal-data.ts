...
async function mkCodeTags(markdownSrc:string):Promise<string> {
...
async function getCodeSample(prpslId:string):Promise<(string | undefined)> {
  ...
  let rawReadmeUrl;
  const result:any = await ghFileImporter
...
async function hasSpec(prpslId:string):Promise<boolean> {
  let isFound = false;
  const result:any = await ghFileImporter.fetchPathMetadata('tc39', prpslId);  
...
async function getShortDescription(prpslId:string):Promise<(string | undefined)> {
  ...
  if (!description) {
    ...
    const repoMetadata:any = await ghFileImporter.fetchRepoMetadata('tc39', prpslId);
